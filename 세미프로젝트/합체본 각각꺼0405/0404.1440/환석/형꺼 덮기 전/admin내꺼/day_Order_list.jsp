	...
	<%for (int i = 0; i < dayOrderList.size(); i++) { %>
			<tr>
				...
				<td>
					...
					<a href="orderDelete.mc?r_no=<%=dayOrderList.get(i).getR_no()%>&r_cal=<%=orderD2%>&r_statime=<%=dayOrderList.get(i).getR_statime()%>&r_uname=<%=dayOrderList.get(i).getR_uname()%>&r_used=<%=dayOrderList.get(i).getR_used()%>">취소</a>
				</td>
			</tr>
	<%} %>
		</table>
