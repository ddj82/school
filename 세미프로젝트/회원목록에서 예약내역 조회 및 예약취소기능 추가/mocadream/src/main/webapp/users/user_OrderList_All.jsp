...
<body style="background-color: rgb(250, 240, 230)">
	<div
			...
			<section id="ListArea">
				<table>
					...
					<tr>
						...
						%>
						<td><a
							href="orderDelete.mc?r_uname=<%=mc_OrderList.get(i).getR_uname()%>&r_no=<%=mc_OrderList.get(i).getR_no()%>&r_cal=<%=orderD2%>&r_statime=<%=mc_OrderList.get(i).getR_statime()%>&r_used=<%=mc_OrderList.get(i).getR_used()%>">취소</a>
						</td>
						<%
						}
						%>
		...
		<section style="text-align: center;">
			...
			%>
			<a href="myOrderList.mc?page=<%=a%>&id=<%=userId%>">[<%=a%>]
			</a>&nbsp;
			<%
			}
			%>
