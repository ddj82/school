		...
		<% if (memberList != null && listCount > 0) { %>
			<table>
				...
				<tr>
					...
					<% if (state == 1) { %>
					<td>
						<button class="button-link" onclick="location.href='memberWarningAction.mc?id=<%=memberList.get(i).getId()%>'">경고주기</button>
					</td>
					<% } else { %>
					<td></td>
					<% } %>
					...
					<% if (state == 2) { %>
					<td>
						<button class="button-link" onclick="location.href='memberReleaseAction.mc?id=<%=memberList.get(i).getId()%>'">정지해제</button>
					</td>
					<% } else { %>
					<td></td>
					<% } %>
