...
public class MocaDAO {
	...
	public int selectMemberListCount(String search) {
		...
		if (search.equals("정지")) {
			...
			try {
				String sql = "SELECT count(*) FROM MC_USERS WHERE UPPER(ID) LIKE UPPER(?) OR UPPER(PW) LIKE UPPER(?) OR UPPER(NAME) LIKE UPPER(?) OR UPPER(TEL) LIKE UPPER(?) OR UPPER(ADDR) LIKE UPPER(?)";
				ps = con.prepareStatement(sql);
