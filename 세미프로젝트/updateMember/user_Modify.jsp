...
#joinformArea {
	width: 30%; margin: 50px auto; padding: 20px; border: 1px solid #ccc; background-color: #fff;
}
...
input[type="text"],input[type="password"] {
	width: calc(100% - 20px); padding: 8px; margin-top: 5px; border: 1px solid #ccc; border-radius: 5px; box-sizing: border-box;
}

input[type="button"] {
	width: 150px; padding: 8px; border: none; border-radius: 5px; background-color: #007bff; color: #fff; cursor: pointer;
}
...
#idchkbtn {
	position: absolute; width: 80px; height: 33px; top: 6px; bottom: 0; right: 20px; margin: auto 0; border-radius: 3px; border: none; background-color: #007bff; color: #fff; cursor: pointer; font-size: 12px;
}
...
#hiddenDiv {
	display: none; /* 초기에는 숨겨진 상태로 설정 */ width: 200px; height: 100px; background-color: lightblue; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);
}

#errorMessages,#iderrorMessages,#pwerrorMessages,#pwcferrorMessages,#nameerrorMessages,#emailerrorMessages,#addrerrorMessages,#telerrorMessages,#postcodeerrorMessages {
	color: red; font-size: 12px; margin: 10px 0; display: none; padding-left: 30px; font-weight: bold;
}
</style>
</head>
<body>
	...
	<section id="joinformArea">
		...
		<form action="myPageModifyProAction.mc" onsubmit="return validateForm()" method="post" name="modifyform">
			<table>
				<tr>
					...
					<br> 
					<input type="text" class="id" pattern="[a-zA-Z0-9]+" name="id" id="id" value="<%=article.getId()%>" readonly></td>
				</tr>
				<tr>
					<td><label for="pw">비밀번호</label> 
					<br> 
					<input name="pw" type="password" class="pw" pattern="[a-zA-Z0-9!_\-!@#$%^&*]+" id="pw" value="<%=article.getPw()%>" readonly></td>
				</tr>
				<tr>
					<td><label for="name">이름</label> 
					<br>
					<input name="name" type="text" class="name" pattern="[가-힣a-zA-Z\s]+" maxlength="6" id="name" value="<%=article.getName()%>"></td>
				</tr>
				<tr>
					<td><label for="tel">휴대전화번호</label><br>
					<input name="tel" type="text" class="tel" pattern="(010)-[0-9]{4}-[0-9]{4}" id="tel" value="<%=article.getTel()%>"></td>
				</tr>
				<tr>
					<td><label for="email">이메일</label><br>
					<input name="email" type="email" class="email" pattern="[a-zA-Z0-9_]+[@][a-zA-Z]+[.][a-zA-Z]+[.]*[a-zA-Z]*" id="email" value="<%=article.getEmail()%>"></td>
				</tr>
				<section>
				...
				<tr>
					<td id="btns">
						<button id="registerBtn" class="btn1" type="submit">수정</button>
					</td>
				</tr>
			</table>
		</form>
		<table>
			<tr>
				<td><a href="javascript:history.go(-1)">[취소]</a> <a href="deleteForm.html">[회원탈퇴]</a></td>
			</tr>
		</table>
