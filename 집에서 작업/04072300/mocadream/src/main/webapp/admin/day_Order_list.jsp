...
<% } else { %>
		...
		<%for (int i = 0; i < dayOrderList.size(); i++) { %>
				<tr>
					...
					if (Integer.parseInt(now1) < Integer.parseInt(orderD)) {
						...
						<td>
							<button class="btn" onclick="location.href='orderDelete.mc?r_uname=<%=dayOrderList.get(i).getR_uname()%>&r_no=<%=dayOrderList.get(i).getR_no()%>&r_cal=<%=orderD2%>&r_statime=<%=dayOrderList.get(i).getR_statime()%>&r_used=<%=dayOrderList.get(i).getR_used()%>';">취소</button>
						</td>
					<% } %>
				</tr>
		<%} %>
	...
	<% } else { %>
		<section class="title2">예약 된 룸이 없습니다.</section>
		<br>
	<% } %>
<% } %>
</div>
